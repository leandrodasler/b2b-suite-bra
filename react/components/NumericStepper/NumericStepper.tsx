...
const normalizeMin = (min: number) => (min == null ? -Infinity : min)
const normalizeMax = (max: number) => (max == null ? Infinity : max)
...
const validateValue = (
  ...
  if (isNaN(value) || value == null) {
    if (defaultValue || 0 < min) return min
    if (defaultValue || 0 > max) return max
    return defaultValue
  }

  const parsedValue = parseFloat((value || 0) + '')
  const normalizedValue = isTyping
    ? Math.round(parsedValue / (unitMultiplier || 1))
    : parsedValue
...
const formattedDisplayValue = (
  ...
  if (!isTyping) {
    const multipliedValue =
      Math.round(value * (unitMultiplier || 1) * 100) / 100
    return `${multipliedValue || 0}${parsedSuffix || ''}`
  }

  return `${value || 0}${parsedSuffix || ''}`
}
...
class NumericStepper extends Component<NumericStepperProps> {
  state = {
    inputFocused: false,
  ...
  changeValue = (
    value: string,
    event: { persist: () => void; value: number },
    isTyping: boolean
  ) => {
    const parsedValue = parseFloat((value || 0) + '')
    ...
    const displayValue = validateDisplayValue(
      isTyping ? (value || 0) + '' : (validatedValue || 0) + '',
      minValue,
      maxValue,
      suffix ?? '',
      unitMultiplier || 1,
    ...
    if (this.state.value !== validatedValue && onChange) {
      ...
      event.persist()
      event.value = validatedValue || 0
      onChange(event)
    }
  }

  handleTypeQuantity = (event: any) => {
    this.changeValue(event.target.value, event, true)
  }

  handleIncreaseValue = (event: any) => {
    // event.stopPropagation()
    // event.preventDefault()
    this.changeValue(this.state.value + 1 + '', event, false)
  }

  handleDecreaseValue = (event: any) => {
    // event.stopPropagation()
    // event.preventDefault()
    this.changeValue(this.state.value - 1 + '', event, false)
  }

  handleFocusInput = (e: { target: { select: () => void } }) => {
    e.target.select()
